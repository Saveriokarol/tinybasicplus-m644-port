...
//    Saveriokarol <sasinodiscord@gmail.com>
//
// Contributors:
...
#define kVersion "v0.15.1 (For ATmega644XX)"

// v0.15.1: 2025-01-23
//      Saveriokarol's changes:
//        - Added CLS command
//        - Added I2C/IIC/TWI interface commands:
//          - BTWC: Begin Two-Wire Communication (statement)
//          - TWW:  Two-Wire Write (statement)
//          - ETWC: End Two-Wire Communication (statement)
//          - RTWD: Request Two-Wire Data (statement)
//          - TWR:  Two-Wire Read (function)
...
// For TWI/IIC/I2C commands, we include:
#include <Wire.h>
...
// Saveriokarol's comment: enable this up here for your use :3
...
#endif
  KW_BTWC, KW_TWW, KW_ETWC, KW_RTWD,
#ifdef ARDUINO
#ifdef ENABLE_EEPROM
...
static const unsigned char twierrormsg[]      PROGMEM = "TWI/I2C communication error.";
...
// Coded by Saveriokarol
begintwi:
  expression_error = 0;
  uint8_t addr = (uint8_t)expression();
  if(expression_error || addr > 0x7F) {
    Serial.println("No such address.");
    Serial.println("Legal addresses: 0~127");
    goto run_next_statement;
  }
...
twiwrite:
  expression_error = 0;
  uint8_t data = (uint8_t)expression();
  if(expression_error)
    goto qwhat;

  if (!twiConnected) {
    Serial.println("Not connected yet to device: operation aborted.");
    goto run_next_statement;
  }
...
endtwi:
  unsigned int code = Wire.endTransmission();
  if (code != 0) {
    printmsg(twierrormsg);
    line_terminator();
    Serial.print("TWI: Controller returned ");
    Serial.println(code);
    goto run_next_statement;
  }
  twiConnected = false;
...
reqtwi:
  expression_error = 0;
  uint8_t _addr = (uint8_t)expression();
  if(expression_error)
    goto qwhat;

  expression_error = 0;
  uint8_t count = expression();
  if(expression_error)
    goto qwhat;
